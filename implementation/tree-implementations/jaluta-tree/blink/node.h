 ...
 *       type    level   sopt   prefix      id         keys        offset     next node    
 *     |   1   |   1   |   1   |   1   |     4     |     4     |     4     |      8      | 
 *     |        prefix data        |                          kv pairs                   |
 *     |                                     kv pairs                                    |
 *     |                                     kv pairs                                    |
 *     |                         kv pairs                      |          index          |
 *
...
typedef struct node
{
  ...
  struct node *next;    // pointer to the right sibling
  struct node *first;   // pointer to the first child if level > 0, otherwise NULL
  char         data[0]; // to place the prefix & the index & all the k-v pairs
}node;
...
void node_adjust_many(node *new_node, node *left, node *right, char *okey, uint32_t *olen, char *key, uint32_t *len,
...
void node_change_last_key(node *bn, const void *key, uint32_t len);
