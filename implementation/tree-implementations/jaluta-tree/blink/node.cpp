...
#define get_val_ptr(n, off) (val_t *)(get_key(n, off) + get_len(n, off))
...
node* node_descend_get_corresponding_key(node *n, const void *key, uint32_t len, char*& high_key, uint32_t& high_key_len)
{
  assert(n->level && n->keys && n->pre == 0);
  index_t *index = node_index(n);
  ...
  get_key_info(n, index[first], key1, len1);
  high_key = (char*)key1;
  high_key_len = len1;
...
bool is_it_high_key(node* n, const char* k, uint32_t l) {
  get_key_info(n, node_index(n)[n->keys - 1], key, len);
  return compare_key(k, l, key, len) == 0;
}
...
void get_max_key(node* n, void* k, uint32_t& l) {
  ut_a(n->keys > 0);
  index_t *index = node_index(n);
  get_key_info(n, index[n->keys - 1], key, len);
  memcpy(k, key, len);
  l = len;
}
...
void node_change_last_key(node *n, const void *key, uint32_t len) {
  ut_a(n->keys > 0);
  void* last_key_offset = n->data + n->off - key_byte - value_bytes;

  // if new key length is greater than old key length, we need to move all keys
  // This case isn't implemented yet
  ut_a(*((len_t *)(last_key_offset)) >= (len_t)len);
  *((len_t *)(last_key_offset)) = (len_t)len;
  last_key_offset = (char*)last_key_offset + key_byte;
  memcpy(last_key_offset, key, len);
}
...
int node_insert(node *n, const void *key, uint32_t len, const void *val)
{
  ...
  if (unlikely(is_overflowed(n, len))) {
    if (!node_try_prefix_compression(n, key1, len1)) {
      ut_a(0);
      return -1;
    }
    ...
    key1 = (char *)key + n->pre;
    len1 = len - n->pre;
    ut_a((n->data + (n->off + key_byte + len1 + value_bytes + index_byte)) <= (char *)index);
  }

  // update index
  --index;
  if (likely(low)) {
    memmove(&index[0], &index[1], low * index_byte);
  }
  index[low] = n->off;
...
int node_set_value(node *n, const void *key, uint32_t len, const void *val) {
  if (n->pre) { // compare with node prefix
    ut_a(n->level == 0);
    // TODO: remove this if we can handle key length <= prefix length
    ut_a(len > n->pre);
  ...
  while (low <= high) {
    ...
    if (r == 0) {
      val_t* cvalue;
      if (mid == 0) {
        n->first = reinterpret_cast<node*>(const_cast<void*>(val));
      } else {
        cvalue = get_val_ptr(n, index[mid - 1]);
        *cvalue = *(val_t *)(&val);
      }
      return 1;
    }
...
bool is_overflowed(node* n, int key_len) {
  index_t *index = node_index(n);
  return n->data + (n->off + key_byte + key_len - n->pre + value_bytes + index_byte) > (char *)index;
}

// split half of the node entries from `old` to `new`
void node_split(node *old, node *new_node, char *pkey, uint32_t *plen)
{
  uint32_t left = old->keys / 2, right = old->keys - left;
  index_t *l_idx = node_index(old), *r_idx = node_index(new_node);
  *plen = 0;

  if (old->pre) { // copy prefix
    ut_a(old->level == 0);
    memcpy(new_node->data, old->data, old->pre);
    new_node->pre = old->pre;
    new_node->off = new_node->pre;
    memcpy(pkey, old->data, old->pre);
    *plen = old->pre;
  }


  // we take left - 1, because one key is in "first"
  int left_new_node_val_index = likely(old->level == 0) ? left : left - 1;
  void *fval = get_val(old, l_idx[left_new_node_val_index]);
  
  if (unlikely(old->level)) { // assign first child if it's not a level 0 node
    new_node->first = static_cast<struct node*>(fval);
  } 

  // we first copy all the keys to `new_node` in sequential order,
  // then move the first half back to `old` and adjust the other half in `new_node`
  // the loop has some optimization, it does not have good readability
  uint32_t length = 0;
  r_idx -= old->keys;
  for (uint32_t i = 0, j = old->keys - right; i < old->keys; ++i) {
    r_idx[i] = new_node->off;
    get_kv_info(old, l_idx[i], okey, olen, oval);
    node_insert_kv(new_node, okey, olen, oval);
    if (i == left - 1)
      length = new_node->off - new_node->pre;
    if (i >= j)
      r_idx[i] -= length;
  }

  // copy the first half data, including prefix
  memcpy(old->data + old->pre, new_node->data + new_node->pre, length);
  old->keys = left;
  old->off  = length + old->pre;
  // update `old` index
  l_idx = node_index(old);
  r_idx = node_index(new_node);
  memcpy(l_idx, r_idx, left * index_byte);

  // adjust `new` layout
  new_node->keys = right;
  new_node->off -= length;
  memmove(new_node->data + new_node->pre, new_node->data + new_node->pre + length, new_node->off - new_node->pre);
...
void node_insert_fence(node *old, node *new_node, void *next, char *pkey, uint32_t *plen)
{
  // remove `blink` type to help insert fence key
  old->type &= (~(uint8_t)Blink);

  if (likely(old->level == 0)) {
    index_t *nidx = node_index(new_node);
    ut_a(new_node->keys);
    get_key_info(new_node, nidx[0], key, len);
    memcpy(pkey, key, len);
    *plen = len;
    ut_a(node_insert(old, key, len, next) == 1);
  } else {
    ut_a(node_insert(old, pkey, *plen, next) == 1);
  }
...
bool node_least_key_is_smaller_then_key(node *n, const void *key, uint32_t len)
{
  ut_a(n->level == 0);
...
bool covered(node *n, const void *key, uint32_t len)
{
  get_key_info(n, node_index(n)[n->keys - 1], last, llen);
...
int node_adjust_few(node *left, node *right, char *okey, uint32_t *olen, char *key, uint32_t *len)
{
  ...
  ut_a((char *)r_idx > (right->data + right->off));
...
// move 1/3 from `left` and 1/3 from `right` to `new_node`, it is assured by `node_adjust_few`
// that `left` and `right` have the same prefix, `okey` and `key` are old fence key and replace
// fence key, `nkey` is new fence key
void node_adjust_many( node *new_node, node *left, node *right, char *okey, uint32_t *olen,
  char *key, uint32_t *len, char *nkey, uint32_t *nlen)
{
  ...
  memcpy(new_node->data, left->data, new_node->pre);
  new_node->off = new_node->pre;

  uint32_t lmk = left->keys / 3, rmk = right->keys / 3;
  // get new node's index
  new_node->keys = lmk + rmk;
  index_t *n_idx = node_index(new_node);
  new_node->keys = 0;
  ...
  for (uint32_t i = lk - lmk; i < lk; ++i) {
    ...
    n_idx[new_node->keys] = new_node->off;
    node_insert_kv(new_node, k, l, (const void *)v);
  }
  ...
  // record new fence key
  node_get_whole_key(new_node, 0, nkey, nlen);
...
{
  ut_a(p->depth < max_descend_depth);
  p->nodes[p->depth++] = n;
}
...
std::ostream&  operator<<(std::ostream& stream, const node* n) {
  stream << "node keys:" << n->keys << " level: " << n->level << " type:" << n->type << std::endl;
  index_t *index = node_index(n);
  for (uint i = 0; i < n->keys; ++i) {
    get_key_info(n, index[i], key, len);
    stream << " key: " << (char*)key << " keylen: " << len << std::endl;
  }
...
static void validate(node *n, int is_batch)
{
  ...
  for (uint32_t i = 1; i < n->keys; ++i) {
    ...
    if (is_batch == 0)
      ut_a(compare_key(pre_key, pre_len, cur_key, cur_len) < 0);
    else
      ut_a(compare_key(pre_key, pre_len, cur_key, cur_len) <= 0);
