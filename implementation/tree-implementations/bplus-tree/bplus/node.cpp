...
// for Bplus node, `node_size` is not really node size
...
void free_btree_node(node *n) {
  if (n->level) {
    free_btree_node(n->first_or_prev);
    assert(n->keys);
...
node* node_descend(node *n, const void *key, uint32_t len)
{
  ...
  return likely(first) ? (node *)get_val(n, index[first - 1]) : n->first_or_prev;
}

// find the key in the leaf, return its pointer, if no such key, return 0
// if this is a Bplus node and we need to move right, return -1
int node_search(node *n, const void *key, uint32_t len, void** val)
{
  assert(n->level == 0);
...
bool has_place(node* n, uint32_t len) {
  return likely((n->data + (n->off + key_byte + len - n->pre + value_bytes + index_byte)) <= (char *)node_index(n));
}
...
int node_insert(node *n, const void *key, uint32_t len, const void *val)
{
  ...
  // if (unlikely((low == (int)n->keys) && low && (n->type & Bplus))) {
  //   return -3;
  // }
...
std::ostream&  operator<<(std::ostream& stream, const node* n) {
  ...
  if (n->level > 0) {
    stream << (node*)((char*)n->first_or_prev + node_offset);
    for (uint i = 0; i < n->keys; ++i) {
        stream << (node *)((char*)get_val(n, index[i]) + node_offset);
    }
  }
  stream << std::endl;
  return stream;
...
void node_split(node *old, node *new_node, char *pkey, uint32_t *plen)
{
  ...
  *plen = 0;
  ...
  // if we are at level 0 and this is not a Bplus node, get prefix key
  if (likely(old->level == 0) && (old->type & Bplus) == 0) {
  ...
  if (unlikely(old->level)) { // assign first child if it's not a level 0 node
    new_node->first_or_prev = static_cast<struct node*>(fval);
  ...
  if (old->level == 0) {
    ...
    old->next = (node*)((char *)new_node - node_offset);
    new_node->first_or_prev = (node*)((char *)old - node_offset);
    if (new_node->next) {
      new_node->next->first_or_prev = (node*)((char *)new_node - node_offset);
    }
  }
}

// for Bplus node, insert the first key of `new` as fence key for `old`
void node_insert_fence(node *old, node *new_node, void *next, char *pkey, uint32_t *plen)
{
  // remove `Bplus` type to help insert fence key
  old->type &= (~(uint8_t)Bplus);
  ...
  char *key, uint32_t *len, char *nkey, uint32_t *nlen)
{
  ...
  if (left->level == 0) {
    left->next =  (node*)((char *)new_node - node_offset);;
    new_node->next  =  (node*)((char *)right - node_offset);;
  }
}
...
void node_print(node *n, int detail)
{
  ...
  if (n->level && (n->type & Bplus) == 0)
    ptr += snprintf(ptr, end - ptr, "first_or_prev: %u\n", n->first_or_prev->id);
...
void btree_node_validate(node *n)
{
  ...
  if (n->level) {
    ...
    node_get_whole_key(n->first_or_prev, n->first_or_prev->keys - 1, child_last_key, &child_last_len);
