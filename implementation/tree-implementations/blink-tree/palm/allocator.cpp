...
static inline block* new_block(block *meta)
{
  ...
  int success;
  block *b = static_cast<block*>(block_alloc(meta, s, &success));
  if (likely(success)) {
    #ifdef __linux__
    b->buffer = mmap(0, palm_block_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    #else
    b->buffer = mmap(0, palm_block_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0);
    #endif
    assert(b->buffer != MAP_FAILED);
    b->now = 0;
    b->tot = palm_block_size;
    b->next = 0;
    return b;
  }
  return 0;
...
void* allocator_alloc(size_t size)
{
  ...
  if (unlikely(success == 0)) {
    block *new_bl = new_block(a->meta_curr);
    if (unlikely(new_bl == 0)) {
      block *meta = new_meta_block();
      meta->next = a->meta_curr;
      a->meta_curr = meta;
      new_bl = new_block(a->meta_curr);
      assert(new_bl);
    }
    new_bl->next = a->curr;
    a->curr = new_bl;
