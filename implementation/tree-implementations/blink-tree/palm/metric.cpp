...
static struct clock clock_get_duration(struct clock *old, struct clock* new_clock)
{
  struct clock c;
  c.cpu = new_clock->cpu - old->cpu;
  c.tot = new_clock->tot - old->tot;
  return c;
}
...
typedef struct metric
{
  ...
  int   len;
  std::unordered_map<const char*, struct clock*>* hashmap;
}metric;
...
void init_metric(int num)
{
  ...
  for (int i = 0; i < metric_num; ++i) {
    metrics[i].len = 0;
    metrics[i].hashmap = new std::unordered_map<const char*, struct clock*>();
  }
}
...
void register_metric(int id, const char *name, struct clock *c)
{
  ...
  m->hashmap->insert(std::make_pair(name, c));
}
...
void update_metric(int id, const char *name, struct clock *before)
{
  ...
  metric *m = _get_metric(id);
  struct clock *old = m->hashmap->at(name);
...
void show_metric()
{
  ...
  for (int i = 0; i < m->len; ++i) {
    for (int j = 0; j < metric_num; ++j) {
      ...
      struct clock *c = m->hashmap->at(m->name[i]);
...
void free_metric()
{
  for (int i = 0; i < metric_num; ++i) {
    ...
    for (int j = 0; j < m->len; ++j) {
      struct clock *r = m->hashmap->at(m->name[j]);
      free(r);
    }
    delete(m->hashmap);
  }
